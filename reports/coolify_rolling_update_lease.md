...
| Bind the health port **before** contesting the lease, and before `init_db` | `bot.py` `main()` | A rolling update probes the new container while the old one still owns the lease. A slow or unreachable Supabase must not keep `/live` dark either. |
